...
# decoding data from Honeywell PM sensor
...
while (1) {
	...
	while($len!=32){
		...
		if ($len==4) { $checksum_is+=ord($buffer); };
		if ($len==5) { $checksum_is+=ord($buffer); };
		if ($len==6) { $checksum_is+=ord($buffer);$PM25    = 256*ord($buffer); };
		if ($len==7) { $checksum_is+=ord($buffer);$PM25   += ord($buffer); };
		if ($len==8) { $checksum_is+=ord($buffer);$PM10    = 256*ord($buffer); };
		if ($len==9) { $checksum_is+=ord($buffer);$PM10   += ord($buffer); };
		if ($len==10) { $checksum_is+=ord($buffer); }
		if ($len==11) { $checksum_is+=ord($buffer); }
		if ($len==12) { $checksum_is+=ord($buffer); }
		if ($len==13) { $checksum_is+=ord($buffer); }
		if ($len==14) { $checksum_is+=ord($buffer); }
		if ($len==15) { $checksum_is+=ord($buffer); }
		if ($len==16) { $checksum_is+=ord($buffer); }
		if ($len==17) { $checksum_is+=ord($buffer); }
		if ($len==18) { $checksum_is+=ord($buffer); }
		if ($len==19) { $checksum_is+=ord($buffer); }
